...
class CLIPrompts {
    constructor() {
        this.musicGenres = [
            ...
            'hip hop', 'country', 'folk', 'reggae', 'metal', 'acoustic',
            'indie', 'alternative', 'funk', 'soul', 'r&b', 'punk'
        ];
    ...
    async getGenerateOptions() {
        ...
        const questions = [
            {
                ...
                choices: [
                    { name: 'GPT Açıklama (AI tarafından şarkı sözü üretilir)', value: 'gpt' },
                    { name: 'Özel Şarkı Sözleri (Kendi sözlerinizi yazın)', value: 'custom' }
                ],
                default: 'gpt'
            }
        ];
            ...
            promptQuestions = [
                {
                    ...
                    validate: (input) => input.trim().length > 0 || 'Açıklama boş olamaz!',
            ...
            promptQuestions = [
                {
                    ...
                    validate: (input) => input.trim().length > 0 || 'Şarkı sözleri boş olamaz!',
                    waitUserInput: true
                }
            ];
        }
        ...
        const additionalQuestions = [
            ...
            },
            {
                ...
                transformer: (input) => input ? chalk.cyan(input) : chalk.gray('(başlık otomatik belirlenecek)')
            },
            {
                type: 'confirm',
    ...
    async showGenerateConfirmation(options) {
        ...
        if (options.promptType === 'gpt') {
            console.log(chalk.white('Açıklama: ') + chalk.green(options.gpt_description_prompt));
        } else {
            const preview = options.prompt.substring(0, 100);
            console.log(chalk.white('Şarkı Sözü: ') + chalk.green(preview + (options.prompt.length > 100 ? '...' : '')));
        }
        
        console.log(chalk.white('Müzik Türü: ') + chalk.cyan(options.tags));
        console.log(chalk.white('Başlık: ') + (options.title ? chalk.cyan(options.title) : chalk.gray('Otomatik')));
        console.log(chalk.white('Enstrümantal: ') + (options.make_instrumental ? chalk.yellow('Evet') : chalk.green('Hayır')));
        ...
        try {
            ...
            lines.slice(0, 5).forEach((line, index) => {
                if (index === 0) {
                    ...
                    console.log(chalk.gray(`${index}. `) + chalk.green(line.substring(0, 80) + (line.length > 80 ? '...' : '')));
                }
            });
    ...
    async getCookieSetup() {
        ...
        return await inquirer.prompt([
            ...
            },
            {
                ...
                transformer: (input) => input ? chalk.green('*'.repeat(10) + '...') : chalk.gray('(boş)')
            },
            {
                type: 'input',
                name: 'twocaptcha_key',
                message: '2Captcha API Key (opsiyonel):',
                transformer: (input) => input ? chalk.green('*'.repeat(15) + '...') : chalk.gray('(boş)')
            },
            {
                type: 'confirm',
                name: 'saveConfig',
    ...
    async showProgress(current, total, item) {
        ...
        console.log(`\r${chalk.cyan(`[${progressBar}] ${percentage}%`)} ${chalk.white(item)}`);
    }
}
