...
class CsvReader {
        ...
        return new Promise((resolve, reject) => {
            ...
            createReadStream(filePath)
                ...
                .on('headers', (headers) => {
                    ...
                    if (!hasPromptField) {
                        reject(new Error(`CSV dosyasında prompt alanı bulunamadı! Gerekli alanlar: ${requiredFields.join(' veya ')}`));
                        return;
                    }
                })
                .on('data', (row) => {
                ...
                .on('end', () => {
                    console.log(chalk.green(`✅ ${prompts.length} geçerli prompt okundu`));
                ...
                .on('error', (error) => {
                    ...
                    this.logger.error('CSV okuma hatası', { file: filePath, error: error.message });
                    reject(error);
                });
        });
    }
    ...
    parseRow(row) {
        ...
        const prompt = {
            ...
            gpt_description_prompt: row.gpt_description_prompt ? row.gpt_description_prompt.trim() : promptText,
...
"folk acoustic song about nature",folk,false,"Forest Whispers"`;
    ...
    async validateCsv(filePath) {
        try {
            ...
            // Türleri say
            const genreCounts = {};
            const instrumentalCount = prompts.filter(p => p.make_instrumental).length;
            
            prompts.forEach(p => {
                genreCounts[p.tags] = (genreCounts[p.tags] || 0) + 1;
            });
            
            console.log(chalk.white('Enstrümantal: ') + chalk.green(instrumentalCount));
            console.log(chalk.white('Şarkı sözlü: ') + chalk.green(prompts.length - instrumentalCount));
            ...
            prompts.slice(0, 3).forEach((prompt, index) => {
                console.log(chalk.gray(`  ${index + 1}. ${prompt.gpt_description_prompt || prompt.prompt}`));
            });
    ...
    static showCsvHelp() {
        ...
        console.log(chalk.green('• prompt') + chalk.gray(' - Özel şarkı sözleri (opsiyonel)'));
        console.log(chalk.green('• gpt_description_prompt') + chalk.gray(' - GPT ile şarkı açıklaması'));
        console.log(chalk.green('• tags') + chalk.gray(' - Müzik türü (pop, rock, jazz, vs.)'));
        console.log(chalk.green('• make_instrumental') + chalk.gray(' - Enstrümantal müzik (true/false)'));
        console.log(chalk.green('• title') + chalk.gray(' - Şarkı başlığı (opsiyonel)'));
