...
(function() {
    ...
    function setupEventListeners() {
        ...
        if (generateForm) generateForm.addEventListener('submit', handleGenerateForm);
        ...
        if (downloadSampleBtn) downloadSampleBtn.addEventListener('click', downloadSampleCsv);
            ...
            promptType.addEventListener('change', function(e) {
                ...
                if (promptTextarea) {
                    if (e.target.value === 'gpt') {
                        ...
                        promptTextarea.placeholder = '> kendi_sarki_sozlerinizi_buraya_yazin';
                    }
                }
            });
        }
    ...
    function updateConnectionAscii(status, auth, limit) {
        const ascii = '\n ┌─────────────────────────────┐\n' +
                     ' │     SUNO AI BAGLANTISI      │\n' +
                     ' │                             │\n' +
                     ' │  Durum: [' + status.padEnd(11) + ']       │\n' +
                     ' │  Kimlik: [' + auth.padEnd(10) + ']       │\n' +
                     ' │  Limit:  [' + limit.padEnd(10) + ']       │\n' +
                     ' └─────────────────────────────┘';
        ...
        fetch('/api/status')
            ...
            .then(function(data) {
                ...
                if (data.sunoInitialized) {
                    ...
                    updateConnectionAscii('BAGLANDI', 'AKTIF', data.remainingLimit || 'BILINMIYOR');
                } else {
                    sunoConnected = false;
    ...
    function updateStatusIndicator(data) {
        ...
        if (data.server === 'running') {
            indicator.className = sunoConnected ? 'status-dot connected' : 'status-dot';
        } else {
    ...
    function initializeSuno() {
        ...
        .then(function(data) {
            if (data.success) {
                sunoConnected = true;
                showNotification('BASARILI', 'Suno AI basariyla baglandi!', 'success');
        ...
        .catch(function(error) {
            ...
            showNotification('HATA', error.message || 'Suno AI baglantisi basarisiz', 'error');
    ...
    function handleGenerateForm(e) {
        ...
        if (!sunoConnected) {
            showNotification('UYARI', 'Once Suno AI baglantisi kurulmali!', 'warning');
            return;
        }
        ...
        showProgressModal('SARKI_URETILIYOR', 'AI sarkinizi olusturuyor, lutfen bekleyin...');
        ...
        .then(function(data) {
            if (data.success) {
                showNotification('BASARILI', data.songs.length + ' sarki basariyla uretildi!', 'success');
        ...
        .catch(function(error) {
            ...
            showNotification('HATA', error.message || 'Sarki uretimi basarisiz', 'error');
    ...
    function displayGeneratedSongs(songs) {
        ...
        resultHtml += '<span class="result-title">[URETILEN_SARKILAR_' + songs.length + ']</span>';
        resultHtml += '</div>';
        resultHtml += '<div class="result-content">';
        
        songs.forEach(function(song, index) {
            resultHtml += '[' + (index + 1) + '] ' + (song.title || 'SARKI_' + (index + 1)) + '\n';
    ...
    function handleBatchForm(e) {
        ...
        .then(function(data) {
            if (data.success) {
                showNotification(
                    'BASARILI', 
                    data.promptCount + ' sarki icin toplu uretim baslatildi! Tahmini sure: ' + data.estimatedTime, 
                    'success'
                );
        ...
        .catch(function(error) {
            ...
            showNotification('HATA', error.message || 'Toplu uretim basarisiz', 'error');
    ...
    function handleLyricsForm(e) {
        ...
        .then(function(data) {
            if (data.success) {
                ...
                showNotification('BASARILI', 'Sarki sozu basariyla uretildi!', 'success');
        ...
        .catch(function(error) {
            ...
            showNotification('HATA', error.message || 'Sarki sozu uretimi basarisiz', 'error');
    ...
    function copyLyrics() {
        ...
        if (navigator.clipboard) {
            navigator.clipboard.writeText(lyricsText.textContent).then(function() {
    ...
    function loadSongs() {
        ...
        .then(function(data) {
            if (data.success) {
                ...
                showNotification('BASARILI', data.count + ' sarki yuklendi!', 'success');
        ...
        .catch(function(error) {
            ...
            showNotification('HATA', error.message || 'Sarki listesi yuklenemedi', 'error');
    ...
    function displaySongs(songs) {
        ...
        songs.forEach(function(song, index) {
            ...
            const created = song.created_at ? 
                'OLUSTURMA: ' + new Date(song.created_at).toLocaleString('tr-TR').replace(/[:.]/g, '_') : '';
    ...
    function getSongStatusClass(status) {
        ...
        if (statusLower.includes('complete') || statusLower.includes('success')) return 'completed';
        if (statusLower.includes('error') || statusLower.includes('fail')) return 'failed';
        return 'processing';
    }
    ...
    function downloadSampleCsv() {
        const csvContent = 'prompt,tags,make_instrumental,title\n' +
    ...
    function logTerminalAction(action) {
        ...
        console.log('%c[TERMINAL] ' + action, 'color: #00ff00; font-family: monospace;');
    }
    ...
    function showConsoleArt() {
        ...
        console.log('%c[SISTEM] Terminal arayuzu basariyla yuklendi', 'color: #00ffff; font-family: monospace;');
        console.log('%c[OZELLIK] Tek sarki uretimi', 'color: #00ff00; font-family: monospace;');
        console.log('%c[OZELLIK] Toplu sarki uretimi (CSV)', 'color: #00ff00; font-family: monospace;');
        console.log('%c[OZELLIK] Sarki sozu uretimi', 'color: #00ff00; font-family: monospace;');
        console.log('%c[OZELLIK] Sarki listesi goruntuleme', 'color: #00ff00; font-family: monospace;');
        console.log('%c[OZELLIK] Gercek zamanli durum takibi', 'color: #00ff00; font-family: monospace;');
    }
