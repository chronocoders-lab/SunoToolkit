...
class SunoManager {
    ...
    async initialize() {
        ...
        if (!this.cookie) {
            throw new Error('Cookie bulunamadı! config/settings.json dosyasını kontrol edin.');
        }
    ...
    async generateSong(options = {}) {
        ...
        console.log(chalk.gray(`Prompt: ${payload.gpt_description_prompt || payload.prompt}`));
        console.log(chalk.gray(`Tür: ${tags}`));
        console.log(chalk.gray(`Enstrümantal: ${make_instrumental ? 'Evet' : 'Hayır'}`));
